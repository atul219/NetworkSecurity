...
class DataValidation:
    ...
    def detect_dataset_drift(self, base_df, current_df, threshold = 0.05) -> bool:
        try:
            ...
            drift_reprt_file_path = self.data_validation_config.drift_report_file_path

            dir_path = os.path.dirname(drift_reprt_file_path)
            os.makedirs(dir_path, exist_ok= True)
            write_yaml_file(file_path= drift_reprt_file_path, content= report)
            return status
        except Exception as e:
            raise NetworkSecurityException(e, sys)
